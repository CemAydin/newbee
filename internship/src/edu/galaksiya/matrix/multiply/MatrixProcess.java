...
public class MatrixProcess {
	...
	public void matrixDiv() {// Matrisi daha küçük matrislere
	...
	synchronized public void collect() {// Dağıttığımız matrisleri geri
		...
		for (int i = 0; i < temp1.length; i++) {
			...
			{
				general.matrix[i][j + temp1[0].length] = temp2[i][j];
			}
		}
		for (int i = 0; i < temp3.length; i++) {
			for (int j = 0; j < temp1[0].length; j++) {
				general.matrix[i + temp1.length][j] = temp3[i][j];
			}
		}
		for (int i = 0; i < temp4.length; i++) {
			for (int j = 0; j < temp4[0].length; j++) {
				general.matrix[i + temp1.length][j + temp1[0].length] = temp4[i][j];
			}
		}
		general.write();
